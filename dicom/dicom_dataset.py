...
class DICOMDataset(Dataset):
    def __init__(
        ...
        """
        args:
            name: the name of the dataset. If None, name is determined from 'path'.
            path: the path to the dataset.
        raises:
            ValueError: if neither 'name' or 'path' are set.
            ValueError: if given 'path' not found.
        """
        # Validate arguments.
        if not (name or path):
            raise ValueError(f"Either 'name' or 'path' should be given.")

        self._id = f"{self.type.name}: {name}"
        ...
        if path:
            ...
            dspath = paths.datasets
            if not dspath:
                raise ValueError(f"Env var 'MD_DATASETS' not set. Please specify 'path' manually, or set 'MD_DATASETS' to structured folder path.")

            self._path = os.path.join(dspath, 'dicom', name)
            if not os.path.exists(self._path):
                datapath = os.path.join(self._path, 'data')
                raise ValueError(f"Dataset '{self}' not found. Please specify 'path' manually, or add data to '{datapath}' folder.")

        # Set name.
        self._name = name if name else os.path.basename(path)
